...
func (g *generator) emitLibraryAPI(cfg simConfig) {
	...
	g.raw("    // n * vm.time_step is a DURATION, not a promise of n solver steps: on an")
	g.raw("    // adaptive solver vm.time_step is wherever the controller currently sits,")
	g.raw("    // so it moves between calls and the run_until below may take any number of")
	g.raw("    // internal steps to cover the span. That was always true; what changed with")
	g.raw("    // .tran's single-argument step is the FIRST call's quantum, which is now")
	g.raw("    // vm_boot's cold-start dt (t_step/10, see cold_start_dt) rather than a")
	g.raw("    // deck-supplied initial dt. A host that wants a fixed-size advance should")
	g.raw("    // call HVR_run(seconds) and say so directly.")
